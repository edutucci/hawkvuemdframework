...
export default {
  ...
  computed: {
    maxPage() {
      ...
      if (this.rowsperpagevalue > -1) {
        if (this.rows.length > 1) {
          const size = this.rows.length - 1;
          maxPage = parseInt((size / this.rowsperpagevalue) + 1, 10);
        }
      }
      return maxPage;
    },
    ...
    maxPageRows() {
      const value = (this.rowsperpagevalue * this.currentPage > this.maxRows) ? this.maxRows : this.rowsperpagevalue * this.currentPage;
      return value;
    },
    maxRows() {
  ...
  methods: {
    ...
    setTableRows(rowsperpagevalue) {
      if (rowsperpagevalue > -1) {
        ...
        const min = (max - rowsperpagevalue);
    ...
    selectAllTableRows(val) {
      this.selectedRows = [];
      if (val) {
        for (let rowindex = 0; rowindex < this.tableData.rows.length; rowindex += 1) {
    ...
    getSelectedRows() {
      const rows = [];
      for (let index = 0; index < this.selectedRows.length; index += 1) {
        rows.push({
    ...
    filterRows(query) {
      if (!query || query.length === 0) {
        ...
        this.tableData.rows = [];
        for (let rowIndex = 0; rowIndex < this.tableRows.length; rowIndex += 1) { // this.tableData.rows.forEach(row => {
          const row = this.tableRows[rowIndex];
          for (let colIndex = 0; colIndex < this.columns.length; colIndex += 1) { // this.columns.forEach(col => {
            const col = this.columns[colIndex];
            if (row[col.field]) {
              const colvalue = row[col.field].toString().toLowerCase();
