  ...
  watch: {
    messages(newValue) {
      if (newValue && newValue.length) {
        for (let rat = 0; rat < this.maxRating; rat += 1) {
  ...
  methods: {
    ...
    fillRatings(maxRating, halfpoints) {
      if ((maxRating >= 0) && (maxRating <= this.maxRating)) {
        for (let rat = 0; rat < parseInt(maxRating, 10); rat += 1) {
          this.ratings[rat].icon = 'fas fa-star';
        }
        if (halfpoints) {
          const decimals = Number(maxRating.toString().split('.')[1]) || 0;
          if (decimals > 0) {
            this.ratings[parseInt(maxRating, 10)].icon = 'fas fa-star-half-alt';
            this.currentRatingIndex = (maxRating + 0.5) - 1;
          } else {
            this.currentRatingIndex = (maxRating < 1) ? 0 : parseInt(maxRating, 10) - 1;
          }
        } else {
          this.currentRatingIndex = (maxRating < 1) ? 0 : parseInt(maxRating, 10) - 1;
        }
    ...
    setRating(rat, index) {
      ...
      } else if (this.currentRatingIndex === index) {
