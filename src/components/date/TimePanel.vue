...
export default {
  ...
  methods: {
    ...
    updatePointers() {
      const pH = this.curHour;
      const fH = this.transforms24Hours.find((hour) => hour.text === pH.toString());

      if (fH) {
        this.updateHour(pH, fH.rotateZ, this.curMin);
      }

      const min = this.curMin;
      const minAngle = (360 / 60) * min;
      const rotMin = `rotateZ(${minAngle}deg)`;
    ...
    updateHour(hour, rotateZ, min) {
      ...
      const h12 = this.transforms12Hours.find((hour) => hour.text === h.toString());
      if (h12) {
        this.pointer12H.transform = h12.rotateZ;
        if (min > 30 && min < 50) {
          const localRotate = h12.rotateZDeg + 15;
          const localRotateString = `rotateZ(${localRotate}deg)`;
          this.pointer12H.transform = localRotateString;
        } else if (min >= 50) {
          const localRotate = h12.rotateZDeg + 23;
          const localRotateString = `rotateZ(${localRotate}deg)`;
          this.pointer12H.transform = localRotateString;
        }
      }
